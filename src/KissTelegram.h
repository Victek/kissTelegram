...
// ========== CAUSAS DE FALLO EN ENVÍO ==========
enum SendFailureReason {
  SEND_OK = 0,                    // Envío exitoso
  SEND_FAIL_DISABLED,             // Bot deshabilitado
  SEND_FAIL_RATE_LIMIT,           // Rate limit activo (encolado)
  SEND_FAIL_WIFI_UNSTABLE,        // WiFi inestable (encolado)
  SEND_FAIL_NO_CLIENT,            // No hay cliente activo
  SEND_FAIL_CONNECTION,           // Fallo al conectar a Telegram
  SEND_FAIL_HTTP_ERROR,           // Error HTTP (4xx, 5xx)
  SEND_FAIL_READ_RESPONSE,        // Fallo al leer respuesta
  SEND_FAIL_QUEUE_FULL            // Cola llena (no se pudo encolar)
};
...
public:
  ...
  bool sendMessage(const char* chat_id, const char* text,
                   MessagePriority priority, SendFailureReason* failReason);  // Con diagnóstico
  ...
  // ========== REPORTES DE ERROR ==========
  const char* getFailureReasonText(SendFailureReason reason);
  void sendErrorReport(const char* chat_id, SendFailureReason reason,
                       const char* context = nullptr);

  // ========== LOGGING Y DIAGNÓSTICO ==========
  void logEvent(const char* event, const char* details = nullptr);
  void logSocketState(const char* when);
  String getLastEvents(int count = 10);
  void clearEventLog();
