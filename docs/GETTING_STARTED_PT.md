...
**Guia completo para configurar seu ESP32-S3 do zero até sua primeira mensagem do Telegram**
...
5. [Primeiro Upload (Criar partições com Arduino IDE)](#primeiro-upload)
...
Seu novo ESP32-S3 N16R8 chega com um aplicativo de demonstração LED RGB integrado. KissTelegram **substitui completamente a tabela de partições** para maximizar seu flash de 16MB:

| Partição | Espressif Padrão | KissTelegram Personalizado |
|-----------|-----------------|----------------------------|
...
6. Escolha um nome de usuário (deve terminar em `bot`, exemplo: "meuassistente_domestico_bot")
7. **Salve seu Token do Bot** - parece com: `1234567890:ABCdefGHIjklMNOpqrsTUVwxyz`

### Passo 2: Obtenha Seu ID de Chat

**Método 1: Usando um Bot (Mais Fácil)**

1. Procure por `@ChatIDHelperBot` no Telegram
...
2. Abra o navegador e visite:
   ```
   https://api.telegram.org/bot<SEU_TOKEN_BOT>/getUpdates
   ```
   (Substitua `<SEU_TOKEN_BOT>` pelo seu token atual)
...
Guarde-os bem! Você precisará deles em breve.
...
- Usada para **upload inicial de firmware**
- Usada para **modo bootloader**
- Use esta quando o Arduino IDE disser "Conectando..."

**PORTA ESQUERDA (OTG):**
- Usada para **operação normal** após o primeiro upload
- Usada para **segundo upload** (correção de partição)
- Use esta para o Monitor Serial em operação normal
...
6. Se o Arduino IDE estava aberto, **feche e reinicie-o**
...
3. **Configure todas as opções do menu Ferramentas:**

   | Configuração | Valor |
   |---------|-------|
   | **Placa** | 4D Systems gen4-ESP32-S3R8n16 |
   | **USB CDC On Boot** | Habilitado |
   | **Tamanho Flash** | 16MB (128Mb) |
   | **Esquema de Partições** | **Custom (4MB APP/12MB LtlFS)** ⚠️ |
   | **PSRAM** | OPI PSRAM |
   | **Velocidade de Upload** | 921600 |
   | **Apagar Todo o Flash Antes do Upload do Sketch** | **Habilitado** ⚠️ |

   ⚠️ **Configurações críticas** - verifique duas vezes!

4. **Ferramentas** → **Monitor Serial** → Defina velocidade para **115200**
...
**O Problema:**
- Primeiro upload: Arduino usa a **tabela de partições antiga** para escrever firmware
...
- Resultado: Erros de inicialização, erros de partição, travamentos
...
3. **Verifique as configurações**:
   - ✅ Apagar Todo o Flash Antes do Upload do Sketch: **Habilitado**
   - ✅ Esquema de Partições: **Custom (4MB APP/12MB LtlFS)**
   

4. **Ferramentas, Carregar** ou (`Ctrl+U`) (Clique na opção que preferir)
   - ✅ O firmware é carregado.
   - Leva 53.6 segundos ou muito menos se você usar uma fonte de alimentação externa para o ESP32s3 
...
3. **Selecione a nova porta**: Ferramentas → Porta → Selecione a nova porta COM
   - **Importante**: O número da porta mudará! Procure por dados no Monitor Serial para confirmar a porta correta, por exemplo, pressione reset do ESP32s3 até ver resposta de dados
...
6. **Aguarde ~2-3 minutos** (apagando + carregando, depende se você usa fonte de alimentação externa)

7. **Abra o Monitor Serial** - você deve ver agora (se configurou corretamente as credenciais 
em system_setup.h (o system_setup_template renomeado)):
   ```
   ✅ KissTelegram v0.9.x
   ✅ WiFi conectado
   ✅ Bot Telegram habilitado
   ✅ Sistema pronto
   ```

8. **Verifique o Telegram** - você receberá mensagem de boas-vindas:
   ```
...
- **Não precisa** de "Apagar Todo o Flash" mais (a menos que tenha feito mudanças em dados NVRAM)
- Upload uma vez e funciona imediatamente

---

## Arquivos de Configuração

### system_setup.h (Necessário Antes do Primeiro Upload!)

**Antes de compilar:**

1. Navegue até sua pasta KissTelegram
2. Encontre `system_setup_template.h`
3. **Renomeie-o** para `system_setup.h`
...
// Segurança OTA (mude PIN/PUK padrão!)
...
// #define LANG_ES  // Español (Espanhol) - PADRÃO se tudo estiver comentado
```

Escolha seu idioma (descomente) **antes de compilar** para mensagens localizadas.
...
Uma vez que KissTelegram está rodando, você pode atualizar firmware **via Telegram** (sem cabo USB!):

1. Envie `/ota` ao seu bot
2. Digite PIN: `/otapin 0000` (ou seu PIN personalizado)
3. **Envie seu arquivo firmware `.bin`** (arrastar e soltar no Telegram)
...
8. Se não confirmar, ESP32 **automaticamente reverte** para firmware anterior!

📖 **Leia mais:** Veja `README_KissOTA_PT.md` para documentação OTA completa.

---

### Explore Código de Exemplo
...
- ✅ Tratamento de comandos (`/start`, `/help`, `/status`, etc.)
- ✅ Atualizações OTA via Telegram
- ✅ Recuperação de falhas e persistência
...
- Tente cabo USB diferente (alguns são apenas para carga, não dados)

**Problema: "Tempo esgotado aguardando dispositivo" durante upload**
- Porta USB errada! Lembre-se: porta DIREITA para primeiro upload, porta ESQUERDA para segundo
- Segure botão BOOT no ESP32 enquanto clica em Upload, solte após "Conectando..." aparecer

**Problema: Monitor Serial mostra caracteres aleatórios**
- Taxa de baud incorreta. Defina para **115200** no dropdown do Monitor Serial

**Problema: Bot não responde no Telegram**
- Verifique que `system_setup.h` tem Token do Bot e ID de Chat corretos
- Verifique que credenciais WiFi estão corretas
- Abra Monitor Serial e procure por mensagens de conexão WiFi

**Problema: Erro de compilação "Tabela de partições não cabe"**
- Não adicionou partição personalizada ao `boards.txt` corretamente
- Ou não selecionou "Custom (4MB APP/12MB LtlFS)" em Ferramentas → Esquema de Partições
...
- 🐛 **Relatórios de Bugs**: Issues do GitHub (link no README.md principal)
...
```
1. Obter Token do Bot + ID de Chat do Telegram ✅
2. Modificar boards.txt (adicionar partição personalizada) ✅
3. Configurar Arduino IDE (Partição personalizada, Apagar habilitado) ✅
4. Editar system_setup.h (credenciais) ✅
5. Conectar porta USB DIREITA ✅
6. Upload #1 (novas partições)✅
