...
**Complete guide to set up your ESP32-S3 from scratch to your first Telegram message**
...
- **Two USB-C cables** (to alternate between bootloader and OTG ports)
...
Your new ESP32-S3 N16R8 arrives with a built-in RGB LED demo app. KissTelegram **completely replaces the partition table** to maximize your 16MB flash:

| Partition | Default Espressif | KissTelegram Custom |
|-----------|-------------------|---------------------|
| App Space | 1.5 MB | 4.5 MB (3x larger!) |
| File System | 5 MB | 13 MB (2.6x larger!) |
| Total Used | 6.5 MB | 17.5 MB |

That's why the two-upload process is required: **the partition table changes between uploads**.
...
2. Search for `@BotFather` (official bot, has blue checkmark)
...
**The Problem:**
- First upload: Arduino uses the **old partition table** to write firmware
- ESP32 boots: Finds the **new partition table** (from `partitions.csv`)
...
2. **Select port**: Tools → Port → Select the COM port that appears
...
3. **Select new port**: Tools → Port → Select the new COM port
   - **Important**: Port number will change! Look for data in Serial Monitor to confirm correct port, for example, press ESP32s3 reset until you see data response
...
8. **Check Telegram** - you'll receive welcome message:
   ```
...
**Success!** Your ESP32-S3 now runs KissTelegram with correct partitions.
...
- **Don't need** "Erase All Flash" anymore (unless you've made changes to NVRAM data)
...
**⚠️ Security Warning:** Change default PIN (`0000`) and PUK (`00000000`) to your own secrets!
...
// #define LANG_EN  // English (English)
...
1. **Send `/status` to your bot** in Telegram - you'll get detailed status report:
   ```
   📦 KissTelegram v1.x.x
   🎯 SYSTEM RELIABILITY
   ✅ System: RELIABLE
...
3. **Send your firmware `.bin` file** (drag and drop in Telegram)
...
📖 **Read more:** See `README_KissOTA_EN.md` for complete OTA documentation.
...
**Pro tip:** Use `/status` command as your **health monitoring tool** - it's your window into KissTelegram internals!
...
- Windows locked the port. Disconnect USB, wait 5s, reconnect.
...
- Verify `system_setup.h` has correct Bot Token and Chat ID
- Verify WiFi credentials are correct
- Open Serial Monitor and look for WiFi connection messages

**Problem: Compile error "Partition table doesn't fit"**
- Didn't add custom partition to `boards.txt` correctly
...
10. Send /status to verify everything works ✅
```
